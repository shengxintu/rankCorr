...
#' The estimation method \code{"cluster-median"} estimates the between-cluster Spearman's rank correlation using the coefficients from the cumulative probability models of \code{x} and \code{y} on cluster index \code{cluster}. The estimation method \code{"approx"} estimates the between-cluster Spearman's rank correlation using the approximated linear relationship between the total, between-, and within-cluster Spearman's rank correlations.
#' @return a list with following components.
#' \tabular{ll}{
#'   \code{'Total'} \tab the total Spearman's rank correlation, including the estimate (\code{Estimate}), the standard error (\code{SE}), the lower and upper bounds of the confidence interval (\code{Lower, Upper}).\cr
#'   \tab \cr
#'   \code{'Within'} \tab the within-cluster Spearman's rank correlation, including the estimate (\code{Estimate}), the standard error (\code{SE}), the lower and upper bounds of the confidence interval (\code{Lower, Upper}).\cr
#'   \tab \cr
#'   \code{'Between'} \tab the between-cluster Spearman's rank correlation estimated, including the estimation method name, the estimate (\code{Estimate}), the standard error (\code{SE}), the lower and upper bounds of the confidence interval (\code{Lower, Upper}).\cr
#'   \tab \cr
